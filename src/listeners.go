...
func handleTCPClient(conn net.Conn, ch *nbc.NonBlockingChan, counter ConnCounter) {
	...
	if srcAddr == "<nil>" {
		log.Println("Error: handleTCPClient. Address:port == nil")
		return
	}
...
func handleUDPClient(conn *net.UDPConn, ch *nbc.NonBlockingChan, counter ConnCounter) {
	/*
	// CHANGED: new code
	srcAddr := conn.RemoteAddr().String()
	if srcAddr == "<nil>" {
		log.Println("Error: handleUDPClient. Address:port == nil")
		return
	}

	addr := strings.Split(srcAddr, ":")

	dp := protocols.GetUDPDesPort(addr, ch)

	if dp == -1 {
		log.Println("Warning. Packet dropped! [UDP] destPort == -1")
		return
	}	

	log.Printf("New UDP connection from %s to port %d -> glutton:?????\n", addr[0], dp)
	...
	for {
		...
		c := net.UDPConn{conn, addr, ch, b, n}
...
func UDPListener(ch *nbc.NonBlockingChan, wg *sync.WaitGroup) {
	...
	CheckError("[*] ResolveUDPAddr Error.", err)
	
	// Listener for incoming UDP connections
	conn, err := net.ListenUDP("udp", addr)
	CheckError("[*] Error in UDP listener", err)
	handleUDPClient(conn, ch)//, &counters)

	*/

	/*for {
		// var b [1500]byte
		// _, _, err := listener.ReadFromUDP(b[0:])
		// if err != nil {
		// 	return
		//}
